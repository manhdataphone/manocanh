...
function App() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const results = await generateProductPhotos(mainImage, refs, settings, setProgressMsg);
      ...
      alert(
        isQuota 
        ? `LỖI HẾT HẠN MỨC (429):\n\nAPI Key hiện tại đã đạt giới hạn. \n\nVui lòng nhấn vào biểu tượng chìa khóa ở góc trên và chọn API Key từ dự án có bật thanh toán (Paid Project).`
        : `LỖI HỆ THỐNG: ${error.message || 'Có lỗi xảy ra.'}`
      );
    } finally {
